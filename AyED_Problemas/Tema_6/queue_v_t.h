...
//Copiar el contenido de uan cola sin modificar la original
template <class T>
void queue_v_t<T>::copy(const queue_v_t<T> &src) {
  front_ = 0;
  back_ = -1;
  for (int i = src.front_; i <= src.back_; i++)	  
    push(src.v_.at(i));
}

//Comprobar si un dato pertenece a una cola;
template <class T> bool queue_v_t<T>::contains(const T& dato) const {
