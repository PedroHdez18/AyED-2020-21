...
{
  assert(v.get_size() == w.get_size());             //Comprobar que los vectores son del mismo tamaño
  rational_t aux = 0;
  double suma = 0;
  for (int i = 0; i < v.get_size(); ++i){
      aux = (v[i] * w[i]);
...
template<class T>
void
vector_t<T>::reves(const vector_t<T>& v){
  cout << "Modificacion: \n";
  double aux[v.get_size()];
  for (int i = 1, j = 0; i <= v.get_size()-1; i++, j++) {
    aux[j] = v[i];
  }
  aux[v.get_size()-1] = v[0];
  for (int i = 0; i < v.get_size(); i++){
    cout << aux[i] << "  ";
  }
}
